...
def load_dist(path):
    if os.path.exists(path):
        logger.info('Loading dist from : {}'.format(path))
...
class ReidEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        self.features.append(outputs.to('cpu'))
    ...
    def compute_dist_msmt(self, A, block_size=1000):
        result = torch.zeros((A.shape[0], A.shape[0]))
        for i in range(0, A.size(0), block_size):
            end_i = min(i + block_size, A.size(0))
            for j in range(0, A.size(0), block_size):
                end_j = min(j + block_size, A.size(0))
                result[i:end_i, j:end_j] = torch.mm(A[i:end_i], A[j:end_j].T)
    ...
    def evaluate(self):
        ...
        if self.cfg.TEST.RERANK.ENABLED:
            ...
            logger.info('Calculating q_q dist')
            q_q_dist = self.cal_dist(self.cfg.TEST.METRIC, query_features, query_features)

            logger.info('Calculating g_g dist')
            g_g_dist = self.compute_dist_msmt(gallery_features)
