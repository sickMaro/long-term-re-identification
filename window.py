...
class Window:
    ...
    __DIRECTORY: str = "C:/Users/robid/PycharmProjects/pythonProject2/"
    first = 0
    ...
    def __init__(self) -> None:
        ...
        self.__btn_identification: tk.Button = tk.Button(self.__frm_video, text="Identify", bg="dark grey",
                                                         relief="ridge", command=self.__show_results)
    ...
    def __configure_and_grid_widgets(self) -> None:
        ...
        self.__btn_identification.grid(row=2, column=0, sticky="ew")
    ...
    def __load_images_and_cameras(cls) -> None:
        ...
        try:
            ...
            if not cls.__CAMERAS:
                pt = cls.__DIRECTORY + "video"
                for file in os.listdir(pt):
                    path = os.path.join(pt, file)
    ...
    def __create_results_frames(self):
        pt: str = self.__DIRECTORY + "results"
        pattern: str = r'(SAT_Camera\d+)_MILLIS_(\d+)_TRK-ID_(\d+)_TIMESTAMP_(\d+-\d+-\d+)'
        children_list: list[tk.Widget] = self.__frm_final_results.winfo_children()
        try:
            for i, (file, child) in enumerate(zip_longest(os.listdir(pt), children_list[1:])):
                if file is None:
                    child.destroy()
                    ...
                    if os.path.isfile(path):
                        ...
                        number = "N: " + str(i + 1)
                        id_ = "ID: " + result.group(3)
                        camera = "CAM: " + result.group(1)
                        time = "TIME: " + datetime.strptime(result.group(4), "%H-%M-%S").strftime("%H:%M:%S")
    ...
    def __on_drag(self, event: tk.Event) -> None:
        if self.__cnv_video is not None:
            ...
            self.__cnv_video.create_image((0, 0), anchor=tk.NW, image=photo)
            self.__current_frame_PI = photo
            self.__current_selected_area = ImageTk.PhotoImage(image.crop((x0, y0, x1, y1)))
    ...
    def __show_results(self):
        if self.__current_selected_area is not None:
            self.__lbl_title.config(text=Window.__RESULT_TITLE)
            self.__configure_window(multiplier=1.8)
            self.__frm_video.grid_remove()
            self.__frm_probe.grid()
            self.__lbl_probe.config(image=self.__current_selected_area)
            ...
            self.__canvas_cameras.bind("<Configure>",
                                       lambda event: self.__canvas_cameras.itemconfig("result",
                                                                                      width=event.width))

            self.__create_results_frames()
